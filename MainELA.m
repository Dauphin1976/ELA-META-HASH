...
% Choose settings for the image
% 1 true, 0 false
ImageSettings(1).OrigFileName = 'tats.jpg';
ImageSettings(1).MultiplierELA = 18;
ImageSettings(1).CompressionQuality = 75;
ImageSettings(1).ShowOriginal = 1; 
ImageSettings(1).ShowInversion = 1;
ImageSettings(1).ShowELA = 1;
ImageSettings(1).ShowGrayELA = 1;
ImageSettings(1).ShowGrayELAfiltered = 1;
ImageSettings(1).GrayELAfilter = 250;
ImageSettings(1).ShowOnlyOrigWhereELA = 1;
ImageSettings(1).OnlyOrigELAfilter = 250;


% choose functions to call
HashResults = CheckHashes(ImageSettings(1))
ImageMetaData = imfinfo(ImageSettings(1).OrigFileName)
GetELA(ImageSettings(1));


function GetELA = GetELA(ImgSet)

    % open file
    OrigImg = imread(ImgSet.OrigFileName);
    
    if (ImgSet.ShowOriginal) 
        figure
        imshow(OrigImg)
    end
    
    if (ImgSet.ShowInversion) 
        figure
        imshow(imcomplement(OrigImg))
    end
    
    % save a compressed copy
    TempImgFile = 'TempImg.jpg';
    imwrite(OrigImg,TempImgFile,'jpg','Quality',ImgSet.CompressionQuality);
    TempImg = imread('TempImg.jpg');

    %compare the differance
    idiff = ImgSet.MultiplierELA*imabsdiff(OrigImg,TempImg);
    ELADIFF = imfuse(OrigImg,idiff,'montage');

    %crop image
    s = size(ELADIFF);
    ELADIFF = imcrop(ELADIFF,[(s(2)/2)+1 0 s(2) s(1)]);
    ...
    ELADIFFGRAY = adapthisteq(rgb2gray(ELADIFF));
    
    if(ImgSet.ShowGrayELA)
        figure 
        imshow(ELADIFFGRAY)
    end
    
    if(ImgSet.ShowGrayELAfiltered)
        mask = ELADIFFGRAY > ImgSet.GrayELAfilter;
        maskedImage = ELADIFFGRAY;
        maskedImage(~mask) = 0;	% Zero out where mask is 0.
        figure 
        imshow(maskedImage)
    end

    if(ImgSet.ShowOnlyOrigWhereELA)
        mask = ELADIFF > ImgSet.OnlyOrigELAfilter;
        maskedImage = OrigImg;
        maskedImage(~mask) = 0;	% Zero out where mask is 0.
        figure 
        imshow(maskedImage)
...
function FindHash = CheckHashes(ImgSet)
    ImgSet.Input = 'file'; % used for hash checking
    HashResults.FileName = ImgSet.OrigFileName;
    ImgSet.Method = 'MD5';
    HashResults.MD5 = DataHash(ImgSet.OrigFileName, ImgSet);
    ImgSet.Method = 'SHA-1';
    HashResults.SHA1 = DataHash(ImgSet.OrigFileName, ImgSet);
    ImgSet.Method = 'SHA-256';
    HashResults.SHA256 = DataHash(ImgSet.OrigFileName, ImgSet); 
    ImgSet.Method = 'SHA-512';
    HashResults.SHA512 = DataHash(ImgSet.OrigFileName, ImgSet); 

    FindHash = HashResults; 
end
% ******************************************************************************
